...
Strategy1 <- function(n, simulations_df,
  ...
  full_data$inflBZ_phase1 <- inf_fun_logit(fit_phase1)[,"Z_obs"]
  ...
  infcal <- survey::calibrate(mydesign, formula = ~inflBX1_phase1 + inflBX2_phase1 + inflBZ_phase1 + 
                                strata, 
...
Strategy2 <- function(n, simulations_df, scenario, N = 10000){
  ...
  # Calibrate
  calibrated_twophase_wave1 <- calibrate(twophase_design_wave1,
                                            ~ inflBX1_phase1 +
                                              inflBX2_phase1 + inflBZ_phase1 +
                                              strata,
                                            phase = 2,
                                            calfun = "raking")
  ...
  fit_wave1 <- svyglm(Y ~ X1 + X2 + Z, family = quasibinomial,
                        design = calibrated_twophase_wave1)
  ...
  resid_model_BX1_wave1 <- lm(inflBX1 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave1,
                              weights = phase2_weight,
                              na.action = na.exclude)
  resid_model_BX2_wave1 <- lm(inflBX2 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave1,
                              weights = phase2_weight,
                              na.action = na.exclude)
  ...
  # Get residuals
  phase2_wave1$residBX1_wave1 <- resid(resid_model_BX1_wave1)
  phase2_wave1$residBX2_wave1 <- resid(resid_model_BX2_wave1)
  
  
  # Re-calculate allocations
  allocation1 <- allocate_wave(phase2_wave1,
                               strata = "strata",
                               y = "residBX1_wave1", method = "iterative",
  ...
  # Calibrate
  calibrated_twophase_wave2 <- calibrate(twophase_design_wave2,
                                         ~ inflBX1_phase1 +
                                           inflBX2_phase1 + inflBZ_phase1 +
                                           strata,
                                         phase = 2,
  ...
  fit_wave2 <- svyglm(Y ~ X1 + X2 + Z, family = quasibinomial,
                      design = calibrated_twophase_wave2)
  ...
  # Regress Latest IFs (computed above) on Phase 1 IFs
  resid_model_BX1_wave2 <- lm(inflBX1 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave2,
                              weights = phase2_weight,
                              na.action = na.exclude)
  resid_model_BX2_wave2 <- lm(inflBX2 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave2,
                              weights = phase2_weight,
                              na.action = na.exclude)
  ...
  # Get residuals
  phase2_wave2$residBX1_wave2 <- resid(resid_model_BX1_wave2)
  phase2_wave2$residBX2_wave2 <- resid(resid_model_BX2_wave2)
  ...
  # Calibrate
  calibrated_twophase_wave3 <- calibrate(twophase_design_wave3,
                                         ~ inflBX1_phase1 +
                                           inflBX2_phase1 + inflBZ_phase1 +
                                           strata,
                                         phase = 2,
  ...
  fit_wave3 <- svyglm(Y ~ X1 + X2 + Z, family = quasibinomial,
                      design = calibrated_twophase_wave3)
  ...
  # Regress Latest IFs (computed above) on Phase 1 IFs
  resid_model_BX1_wave3 <- lm(inflBX1 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave3,
                              weights = phase2_weight,
                              na.action = na.exclude)
  resid_model_BX2_wave3 <- lm(inflBX2 ~ inflBX1_phase1 +
                              inflBX2_phase1 + inflBZ_phase1 + strata,
                              data = phase2_wave3,
                              weights = phase2_weight,
                              na.action = na.exclude)
  ...
  # Get residuals
  phase2_wave3$residBX1_wave3 <- resid(resid_model_BX1_wave3)
  phase2_wave3$residBX2_wave3 <- resid(resid_model_BX2_wave3)
...
Strategy3 <- function(n, simulations_df, scenario, N = 10000){
  ...
  ## Here, each wave chooses n/4 sample. First two waves are optimal for BX1,
  ## Last two for BX2
  ...
  infcal <- survey::calibrate(mydesign, formula = ~ inflBX1_phase1 +
                                inflBX2_phase1 + inflBZ_phase1 + strata, phase = 2, calfun = "raking")
...
Strategy4 <- function(n, simulations_df, scenario, N = 10000){
  ...
  XZY_obs <- mvrnorm(N, c(0,0,0,0),
                     matrix(c(error_varX1, 0.03, 0.02, 0.02,
                              ...
                              0.02, 0.05, 1, 0,
                              0.02, 0.4, 0, 1), nrow = 4))
  #full_data$X1_obs <- full_data$X1 + 0.2*full_data$X1 +
  #  errorBX1_Y*full_data$Y + XZY_obs[,1]
  #full_data$X2_obs <- full_data$X2 + errorBX2_Y*full_data$Y + 
  #  0.5*full_data$X2 + XZY_obs[,2]
  full_data$X1_obs <- full_data$X1 + XZY_obs[,1]
  full_data$X2_obs <- full_data$X2 + XZY_obs[,2]
...
Strategy5 <- function(n, simulations_df, scenario, N = 10000){
  ...
  # Step 2: Find true influence functions and get residuals
  data$inflBX1_true <- inf_fun_logit(true_modelY)[,"X1"]
  data$inflBX2_true <- inf_fun_logit(true_modelY)[,"X2"]
  
  resid_model_BX1 <- lm(inflBX1_true ~ inflBX1_phase1 + inflBX2_phase1 +
                          inflBZ_phase1 + strata,
                        data = data,
                        na.action = na.exclude)
  resid_model_BX2 <- lm(inflBX2_true ~ inflBX1_phase1 + inflBX2_phase1 +
                          inflBZ_phase1 + strata,
                        data = data,
                        na.action = na.exclude)
  
  data$residBX1 <- resid(resid_model_BX1)
  data$residBX2 <- resid(resid_model_BX2)
  
  
  # Step 3: Determine optimum allocation for phase 2
  allocation <- optimum_allocation(data,
                                           strata = "strata",
                                           nsample = n,
                                           y = c("residBX1", "residBX2"),
                                           weights = c(0.5, 0.5))
