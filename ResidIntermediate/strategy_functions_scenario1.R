...
Strategy1 <- function(n, simulations_df,
  ...
  data$Y1 <- ifelse(data$sample_indicator == 1, full_data$Y1 , NA)
  data$Y2 <- ifelse(data$sample_indicator == 1, full_data$Y2 , NA)
...
Strategy2 <- function(n, simulations_df, scenario, N = 10000){
  ...
  #### Compute Phase 1 influence functions
  fitY1_phase1 <-  glm(Y1_obs ~ X_obs + Z1_obs + Z2_obs + Y2_obs, 
                       family = "binomial", data = full_data)
  fitY2_phase1 <-  glm(Y2_obs ~ X_obs + Z1_obs + Z2_obs, 
                       family = "binomial", data = full_data)
...
Strategy3 <- function(n, simulations_df, scenario, N = 10000){
  ...
  #### Calculate naive influence functions
  fitY1_phase1 <-  glm(Y1_obs ~ X_obs + Z1_obs + Z2_obs + Y2_obs, 
                       family = "binomial", data = full_data)
  fitY2_phase1 <-  glm(Y2_obs ~ X_obs + Z1_obs + Z2_obs, 
                       family = "binomial", data = full_data)
  ...
  # Get IFs
  inflB12_wave2 <- inf_fun_logit(fitY2_wave2)
  infl_wave2 <- data.frame(as.numeric(rownames(inflB12_wave2)), 
                           inflB12_wave2[,"X"])
  ...
  # Get IFs
  inflB12_wave3 <- inf_fun_logit(fitY2_wave3)
  infl_wave3 <- data.frame(as.numeric(rownames(inflB12_wave3)), 
                           inflB12_wave3[,"X"])
...
Strategy3.5 <- function(n, simulations_df, scenario, N = 10000){
  ...
  #### Compute phase1 influence functions
  fitY1_phase1 <-  glm(Y1_obs ~ X_obs + Z1_obs + Z2_obs + Y2_obs, 
                       family = "binomial", data = full_data)
  fitY2_phase1 <-  glm(Y2_obs ~ X_obs + Z1_obs + Z2_obs, 
                       family = "binomial", data = full_data)
  ...
  if(any(wave1_allocation$stratum_size <= 2)){
    ...
    missing_n <- sum(2 - wave1_allocation[wave1_allocation$stratum_size <= 2, "stratum_size"])
    largest_strata <- wave1_allocation[wave1_allocation$stratum_size == max(wave1_allocation$stratum_size),
                                       "strata"][1]
    wave1_allocation[wave1_allocation$strata == largest_strata,
                     "stratum_size"] <- wave1_allocation[wave1_allocation$strata == largest_strata,
                                                        "stratum_size"] - missing_n
    wave1_allocation[wave1_allocation$strata %in% small_strata, "stratum_size"] <- 2
  }
  ...
  # Get IFs
  inflB12_wave1 <- inf_fun_logit(fitY2_wave1)
  infl_wave1 <- data.frame(as.numeric(rownames(inflB12_wave1)), 
                           inflB12_wave1[,"X"])
...
Strategy5 <- function(n, simulations_df, scenario, N = 10000){
  ...
  full_data <- full_data |>
    dplyr::mutate(Y1_strat = Y1_obs, Y2_strat = Y2_obs, X_strat = X_obs,
                  sum_abs_inf = abs(inflB11) + abs(inflB12)) |>
    split_strata(strata = c("Y1_strat", "Y2_strat"),
                 split_var = "X_obs",
                 type = "local quantile",
                 split_at = c(0.25, 0.75),
