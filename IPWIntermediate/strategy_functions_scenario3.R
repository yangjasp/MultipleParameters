...
Strategy1 <- function(n, simulations_df,
  ...
  output <- c(fitY1$coefficients["X1"], fitY1$coefficients["X2"],
              ...
              cor_Y1_Y2,
              prev_Y1, prev_Y2, B11, B21, B12, B22, corX1X2)
  return(output)
}
...
Strategy2 <- function(n, simulations_df, scenario, N = 10000){
  ...
  phase1_data$inflB11 <- inf_fun_logit(fitY1_phase1)[,"X1_obs"]
  phase1_data$inflB21 <- inf_fun_logit(fitY1_phase1)[,"X2_obs"]
  phase1_data$inflB12 <- inf_fun_logit(fitY2_phase1)[,"X1_obs"]
  phase1_data$inflB22 <- inf_fun_logit(fitY2_phase1)[,"X2_obs"]
  
  # Step 4: Determine optimum allocation for wave 1 with Wright algorithm
  allocation1 <- optimum_allocation(phase1_data,
                                    strata = "strata",
                                    y = "inflB11",
                                    nsample = n/16, method = "WrightI")
  ...
  phase1_data <- subset(phase1_data, select = -c(inflB11, inflB21,
                                                 inflB12, inflB22))
